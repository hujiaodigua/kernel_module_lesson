/*************************************************************************
	> File Name: hello_pcie.c
	> Author: 
	> Mail: 
	> Created Time: Tue 04 Aug 2020 11:26:14 AM +08
 ************************************************************************/
...
MODULE_DESCRIPTION("pcie device dirver");
...
static struct calss *hello_class;
...
// barn(bar0,bar1 etc. n = 0,1,2或者0,1,2,3,4,5) 空间的物理地址, 长度, 虚拟地址
...
#define HELLO_VENDOR_ID 0x666    // 假设一个VID
#define HELLO_DEVICE_ID 0x999    // 假设一个PID
...
{
    // 下面几步，在手册中有exampleexample
   
    // iATU_write_config_dword(my_device.pci_dev,iATU Lower Target Address ,xxx);
    // xxx表示内存中的地址，将bar0映射到这块内存映射到这块内存    
   
    // iATU_write_config_dword(my_device.pci_dev,iATU Upper Target Address ,xxx);
    // xxx表示内存中的地址，将bar0映射到这块内存
...
static int hello_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    // 假设bar0是作为内存,流程是这样的,但在本程序中不对bar0进行任何操作
    ...
    // 申请一块DMA内存,作为源地址,在进行DMA读写时会用到
    dma_src_vir = (dma_addr_t)pci_alloc_consistent(pdev, DMA_BUFFER_SIZE, &dma_dst_phy);
    if (dma_dst_vir != 0)
    {
...
static int hello_drv_init(void)
{
    ...
    if (ret < 0)
    {
        printk("failed: pci_chrdev_region\n");
        return ret;
    }
